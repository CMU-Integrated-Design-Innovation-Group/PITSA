...
The sampling-based exhaustive search method: The user needs to enter a number (e.g., 1e-4 for the two-dimensional problem and 500 for the four-dimensional problem) to define the distance between two adjacent sample points.  

The region-based exhaustive search method: The user needs to enter a number (e.g., 1e-4 for the two-dimensional problem and 500 for the four-dimensional problem) to define the edge length of each square/hypercube. The user also needs to specify the number of subintervals in each dimension (e.g., 3, 5, or 10) for the splitting approach that is used to derive the sharper bounds for interval computation. 
